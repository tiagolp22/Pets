        ...
        <div class="grid grid-cols-4 gap-6">
            <aside class="col-span-1 bg-gray-200 p-4 rounded-lg shadow-lg">
                ...
                <form method="GET" action="{{ route('produit.index') }}">
                    ...
                    <div class="mb-4">
                        <h3 class="font-semibold mb-2">{{ __("produits.index.brand") }}</h3>
                        <ul class="space-y-2">
                            <li>
                                <label class="flex items-center">
                                    <input type="checkbox" name="marque[]" value="Marque A" class="mr-2">
                                    {{ __("produits.index.brand_a") }}
                                </label>
                            </li>
                            <li>
                                <label class="flex items-center">
                                    <input type="checkbox" name="marque[]" value="Marque B" class="mr-2">
                                    {{ __("produits.index.brand_b") }}
                                </label>
                            </li>
                        </ul>
                    </div>
                    <div>
                        <h3 class="font-semibold mb-2">{{ __("produits.index.other_filters") }}</h3>
                        <ul class="space-y-2">
                            <li>
                                <label class="flex items-center">
                                    <input type="checkbox" name="autres[]" value="naturel" class="mr-2">
                                    {{ __("produits.index.natural") }}
                                </label>
                            </li>
                            <li>
                                <label class="flex items-center">
                                    <input type="checkbox" name="autres[]" value="bio" class="mr-2">
                                    {{ __("produits.index.organic") }}
                                </label>
                            </li>
                        </ul>
                    </div>
                    <div class="flex justify-between">
                        <button type="submit" class="bg-orange-500 text-white px-4 py-2 rounded">{{ __("produits.index.apply") }}</button>
                        <button type="reset" class="bg-gray-300 text-black px-4 py-2 rounded">{{ __("produits.index.clear") }}</button>
                    </div>
                </form>
            </aside>
